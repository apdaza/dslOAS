...
public class GeneradorListWeb {
  ...
  public GeneradorListWeb(final Resource resource, final IFileSystemAccess fsa) {
	...
	public CharSequence compilarTexto(final Entidad entidad) {
	    ...
	    CharSequence _compilarCampos = this.compilarCampos(_campos, entidad);
	    this.fsa.generateFile("views/"+entidad.getNombre()+".html", _compilarCampos);
	    _builder.newLineIfNotEmpty();
	    return _builder;
	  }
	
	public CharSequence compilarCampos(final List<Campo> campos, final Entidad entidad) {
	    StringConcatenation _builder = new StringConcatenation();
	    _builder.append("<div ng-controller=\'"+ entidad.getNombre() + "Ctrl\'>");
	    _builder.newLine();
	    _builder.append("\t");
	    _builder.append("<table id='"+entidad.getNombre()+"' class=\'table table-striped table-bordered\'>");
	    _builder.newLine();
	    _builder.append("\t");
